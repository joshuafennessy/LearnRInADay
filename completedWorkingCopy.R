...
#remember you can use ??unzip to search online for help documentation
...
nyc_taxi2 <- read_csv(taxiFileName)
...
excelfile <- "https://github.com/joshuafennessy/onedayrworkshopdata/blob/master/samples/bank_holidays.xlsx"
...
#the weather data.
weather_df <- sqlFetch(channel=myChannel, sqtable="weather.dailyrecords", errors=TRUE)

  
  ## use this space to work with the holidays_df data set and answer the following questions
  ##
  ## 1. How many days had precipitation?
          length(weather_df$prcp[weather_df$prcp > 0.0])
  ## 2. What was the average temp on July 4?
          weather_df[weather_df$yyyymmdd == 20160704,]
  ## 3. What day had the lowest temperature?
          #weather_df$ 
          
##if you are unable to connect to the database, you can use the code here to 
##load the data from a csv
  weather_df <- read_csv("ny_weather.csv")
...
#####
#
#  Operation 5: Select columns with dplyr
#
...
#####
#
# Operation 6: Filtering rows with dplyr
#
...
#####
#
# Operation 7: Merging data with dplyr using mutuate and joins
# 
...
#####
#
# Operation 8: Feature engineering
# 
...
# the dplyr function sample_freq() helps here
